...
public function addCart()
{
    ...
    public function search(){
        if(isset($_POST['search_public'])||isset($_SESSION['search_public'])){
            $gender= 0;
            $style= 0;
            $price= 0; 
            $search_public=array();
            if(isset($_POST['search_public'])){
                ...
                $style= $_POST['style'];
                $price= $_POST['price']; 
                $search_public['gender']=$gender;
                $search_public['style']=$style;
                $search_public['price']=$price; 

                Session::createSession('search_public',$search_public);
            } else {
                $search_public=Session::getSession('search_public');
            ...
            $link_full='search?p={page}';
            $limit = 12;
            $count=count($products);
            $current_page = isset($_GET['p']) ? $_GET['p'] : 1;
            $paging = new Pagination();
            $paging->init($current_page, $limit, $link_full, $count);
            $products_info=Products_info::allPaginationSearch($current_page,$limit,$search_public);
            $cats = Category::all();
            $sizes = Sizes::all();
            $hot_product = Products_info::getHotProduct();
            $gender_men_cats=Category::find('gender',1);
            $gender_women_cats=Category::find('gender',0);
            return view('public/index',['search_public'=>$search_public,'products_info' => $products_info,
              'cats' => $cats, 'sizes' => $sizes, 
              'hot_product' => $hot_product,'gender_men_cats'=>$gender_men_cats,'gender_women_cats'=>$gender_women_cats,'paginghtml'=>$paging->html()]); 
        } else {
            return redirect('');
